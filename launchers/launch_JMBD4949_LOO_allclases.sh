...
#        for numfeat in "${numfeats[@]}"; do
        for numfeat in $numfeats; do
            python main.py --epochs 50 --work_dir works_LOO_JMBD4949_allFiles/work_${ngf}_numFeat${numfeat} \
            --layers ${ngf} --batch_size 64 --lr 0.1 --optim ADAM \
            --num_workers 0 --seed ${try}  --num_feats ${numfeat} \
            --tr_data work_JMBD4949_loo_allFiles/tfidf_4949_loo.txt \
            --te_data work_JMBD4949_loo_allFiles/tfidf_4949_loo.txt \
            --class_dict work_JMBD4949_loo_allFiles/tfidf_4949_loo_classes.txt --LOO true --classes ${classes}
        done
    done
done
cd launchers