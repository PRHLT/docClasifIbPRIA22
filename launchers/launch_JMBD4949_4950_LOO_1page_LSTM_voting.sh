...
#ngfs=( 0 )
numfeats=$(seq 9 11 | xargs -n 1 -I {} echo "2^"{} | bc)
lr=0.01
opt=ADAM
# model=LSTMvoting
...
for model in "${models[@]}"; do
    for ngf in "${ngfs[@]}"; do
        for numfeat in $numfeats; do
            python main.py --epochs 128 --work_dir works_JMBD4949_4950_loo_1page_other_${model}/work_${ngf}_numFeat${numfeat}_128epochs_${lr}lr${opt} \
            ...
            --classes ${classes} --model ${model} --gpu 0
        done
    done
done
done
