...
ngfs=( 0 128,128 128 )
# ngfs=( 0 )
classes=P,CP,O,A,T,V,R,DP,D,C,TH,other
# classes=P,CP,O,A,T
#ngfs=( 16 32,16 128,64,32  )
# numfeats=$(seq 4 15 | xargs -n 1 -I {} echo "2^"{} | bc)
#ngfs=( 0 )
numfeats=$(seq 6 9 | xargs -n 1 -I {} echo "2^"{} | bc)
#res_10kwords res_10kwords_withoutNormalize res_10kwords_normRWs
path_data=work_tr49_te50_groups_11classes_other
# path_data=work_JMBD4949_4950_loo_1page
for try in $trys; do
    for ngf in "${ngfs[@]}"; do
        for numfeat in $numfeats; do
            python main.py --epochs 500 --work_dir works_tr49_te50_groups_11classes_other/work_${ngf}_numFeat${numfeat} \
